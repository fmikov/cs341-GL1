	...
	*/
	...
	function update_cam_transform(frame_info) {
		...
		*/
		...
		let M_rotationZ = mat4.fromZRotation(mat4.create(), cam_angle_z);
		let M_rotationX = mat4.fromYRotation(mat4.create(), cam_angle_y);
		let M_rot = mat4.create();
		mat4.multiply(M_rot, M_rotationZ, M_rotationX);
		...
		vec3.negate(forward, cam_pos);

		let tmp = vec3.create();
		vec3.set(tmp, 0., 1., 0.);
		let right = vec3.create();
		vec3.cross(right, tmp, forward);

		let up = vec3.create();
		vec3.cross(up, forward, right);
		console.log(tmp);
		//F=Have a fixed camera view and combine it with rotation matrices.
		...
		// Store the combined transform in mat_turntable
		 frame_info.mat_turntable = M_rot;
